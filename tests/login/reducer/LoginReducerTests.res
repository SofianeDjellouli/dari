...
test("Handles Change action", () => {
  ...
  let action = LoginReducer.Change({name: "email", value: "myemail@gmail.com"})

  let result = LoginReducer.reducer(state, action)
...
test("Handles SetState action", () => {
  ...
  let action = LoginReducer.SetState(newState)
